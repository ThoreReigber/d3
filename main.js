...
async function init() {
  ...
  let simplifiedData = data.map((d) => {
    return {
      annualMeanTemp: d['Max_Temperature_of_Warmest_Month'],
      futureAnnualMeanTemp: d['future_Max_Temperature_of_Warmest_Month'],
      city: d['current_city'],
    };
  });
  ...
  const chartWidth = 12000;
  const chartHeight = 800;
  const padding = 10;
  //const paddingBetweenDataMembers = 0;
  ...
  selection
    ...
    .attr('fill', 'yellow') // Sets the color of the bars.
    .attr('x', (value, index) => {
  ...
  selection
    ...
    .attr('fill', 'white') // Sets the color of the bars.
    .attr('y', (value, index) => {
      // Set the X position of the rectangle by taking the index of the current item we are creating, multiplying it by the calculated width of each bar, and adding a padding value so we can see some space between bars.
